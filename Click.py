...
class Click:
    ...
    def get_all_monitors(self) -> List[Tuple[int, int, int, int]]:
        """获取所有显示器的边界信息，返回(left, top, width, height)的列表"""
        monitors = []
        screen_info = pyautogui.size()
        screen_width, screen_height = screen_info
    ...
    def print_monitor_info(self):
        """打印所有显示器的信息"""
        print(f"检测到 {len(self.monitors)} 台显示器:")
        for i, (left, top, width, height) in enumerate(self.monitors):
            print(f"显示器 {i}: 位置=({left}, {top}), 尺寸={width}x{height}")

    def is_window_on_monitor(self, window, monitor_idx: int) -> bool:
        """检查窗口是否在指定的显示器上"""
        if monitor_idx < 0 or monitor_idx >= len(self.monitors):
            return False

        m_left, m_top, m_width, m_height = self.monitors[monitor_idx]
        m_right = m_left + m_width
        m_bottom = m_top + m_height
    ...
    def find_and_activate_window_on_monitor(self, window_title: str, monitor_idx: int) -> Optional[gw.Window]:
        """在指定显示器上查找并激活窗口"""
        windows = gw.getWindowsWithTitle(window_title)
        if not windows:
            return None
        ...
        # 优先选择指定显示器上的窗口
        for window in windows:
            if self.is_window_on_monitor(window, monitor_idx):
                try:
                    window.activate()
                    print(f"已激活显示器 {monitor_idx} 上的窗口: {window_title}")
                    return window
        ...
        try:
            window.activate()
            print(f"已激活窗口: {window_title} (在显示器 {self.get_window_monitor(window)})")
            return window
        except Exception as e:
            print(f"激活窗口时出错: {e}")
            return None

    def get_window_monitor(self, window) -> int:
        """获取窗口所在的显示器索引"""
        w_center_x = window.left + window.width // 2
        w_center_y = window.top + window.height // 2

        for i, (m_left, m_top, m_width, m_height) in enumerate(self.monitors):
            m_right = m_left + m_width
            m_bottom = m_top + m_height
            if (m_left <= w_center_x < m_right) and (m_top <= w_center_y < m_bottom):
                return i
    ...
    def start_application_from_desktop(self, app_name: str, monitor_idx: int) -> bool:
        """从桌面启动指定应用程序"""
        # 获取桌面路径
        desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    ...
    def click_relative_to_window(self, window, x_ratio: float, y_ratio: float) -> bool:
        """相对于窗口点击指定比例位置"""
        if not window:
            print("无法点击，窗口不存在")
            return False
    ...
    def click(self, x_ratio: float = 0.15, y_ratio: float = 0.15, target_monitor_idx: int = 0, app_name: str = "飞书"):
        """
        在主显示器上查找并激活指定应用程序窗口，然后点击相对位置

        Args:
            x_ratio: 相对于窗口宽度的点击位置比例 (0.0-1.0)
            y_ratio: 相对于窗口高度的点击位置比例 (0.0-1.0)
            target_monitor_idx: 目标显示器索引
            app_name: 应用程序名称
        """
        # 获取所有显示器信息
        ...
        # 检查目标显示器是否存在
        if target_monitor_idx < 0 or target_monitor_idx >= len(self.monitors):
            print(f"指定的显示器编号 {target_monitor_idx} 无效，使用默认显示器 0")
            target_monitor_idx = 0

        # 尝试在指定显示器上查找并激活窗口
        window = self.find_and_activate_window_on_monitor(app_name, target_monitor_idx)

        # 如果未找到窗口，则从桌面启动
        if not window:
            print(f"在显示器 {target_monitor_idx} 上未找到 {app_name} 窗口，尝试启动应用程序...")
            if self.start_application_from_desktop(app_name, target_monitor_idx):
                # 再次尝试查找窗口
                window = self.find_and_activate_window_on_monitor(app_name, target_monitor_idx)
                if not window:
                    print("启动后仍未找到窗口，程序退出")
        ...
        # 点击相对于窗口的指定坐标
        return self.click_relative_to_window(window, x_ratio, y_ratio)


# 保持向后兼容的独立函数
def main(target_monitor_idx: int = 0, x_ratio: float = 0.15, y_ratio: float = 0.15):
    """向后兼容的独立函数"""
    clicker = Click()
    return clicker.click(x_ratio, y_ratio, target_monitor_idx)


if __name__ == "__main__":
    # 可以通过命令行参数指定显示器编号和点击位置，默认为0和(0.15, 0.15)
    target_monitor = 0
    x_ratio = 0.15
    y_ratio = 0.15
    ...
    if len(sys.argv) > 2:
        try:
            x_ratio = float(sys.argv[2])
        except ValueError:
            print("x坐标比例必须是浮点数，使用默认值0.15")

    if len(sys.argv) > 3:
        try:
            y_ratio = float(sys.argv[3])
        except ValueError:
            print("y坐标比例必须是浮点数，使用默认值0.15")

    # 防止鼠标失控，提供10秒的准备时间
    print(f"程序将在10秒后开始执行，操作显示器 {target_monitor}，点击位置 ({x_ratio}, {y_ratio})...")
    time.sleep(10)

    # 使用类的方式
    clicker = Click()
    clicker.click(x_ratio, y_ratio, target_monitor)

    # 或者使用独立函数
    # main(target_monitor, x_ratio, y_ratio)